    ...
    <div class="container" style="margin-top: 100px">
        <div class="card col-lg-8 mx-auto">
            <div class="card-header">
                <h3 class="fw-bold">Form Edit Prestasi</h3>
            </div>
            <div class="card-body">
                <form action="{{ route('prestasi.update', $prestasi->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nama_siswa') is-invalid @enderror" id="nama_siswa"
                            name="nama_siswa" value="{{ old('nama_siswa', $prestasi->nama_siswa) }}" required>
                        @error('nama_siswa')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('tingkat') is-invalid @enderror" id="tingkat" name="tingkat"
                                ...
                                <option value="Sekolah"
                                    {{ old('tingkat', $prestasi->tingkat) == 'Sekolah' ? 'selected' : '' }}>Sekolah</option>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('tahun') is-invalid @enderror" id="tahun"
                                name="tahun" value="{{ old('tahun', $prestasi->tahun) }}" required>
                            @error('tahun')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('kategori') is-invalid @enderror" id="kategori" name="kategori"
                            ...
                            <option value="Olahraga"
                                {{ old('kategori', $prestasi->kategori) == 'Olahraga' ? 'selected' : '' }}>Olahraga
                            </option>
                            <option value="Seni" {{ old('kategori', $prestasi->kategori) == 'Seni' ? 'selected' : '' }}>
                                Seni</option>
                            <option value="Lainnya"
                                {{ old('kategori', $prestasi->kategori) == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        @error('kategori')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror

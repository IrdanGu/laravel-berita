    ...
    @section('content')
        ...
        <div class="col-lg-8">
            <form method="post" action="{{ route('dashboard.organization.update', $member->id) }}" class="mb-5"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('position') is-invalid @enderror" id="position"
                        name="position" required value="{{ old('position', $member->position) }}">
                    @error('position')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('order') is-invalid @enderror" id="order"
                        name="order" required value="{{ old('order', $member->order) }}">
                    @error('order')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
